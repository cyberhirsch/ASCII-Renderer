...
fn transmit(ro : vec3f, rd : vec3f, maxT : f32) -> f32 {
  ...
  var trans = 1.0;
  // trees: walk at cell pitch along the ray, one cell per step
  let hlen = length(rd.xy);
  if (hlen > 1e-4) {
    let treeRange = min(maxT, 14.0);
    let steps = i32(min(treeRange * hlen, 14.0));
    var lastCell = vec2i(99999, 99999);
    for (var i = 0; i < steps; i = i + 1) {
      let p = ro + rd * (f32(i) + 0.5) / hlen;
      let cell = vec2i(i32(floor(p.x)), i32(floor(p.y)));
      if (cell.x == lastCell.x && cell.y == lastCell.y) { continue; }
      lastCell = cell;
      let tr = treeAt(cell.x, cell.y);
      if (!tr.present) { continue; }
      let g = terrainH(vec2f(tr.cx, tr.cy));
      let canZ = g + tr.trunkH + tr.r * 0.55;
      if (p.z > g && p.z < g + tr.trunkH &&
          length(p.xy - vec2f(tr.cx, tr.cy)) < 0.14) { return 0.0; }
