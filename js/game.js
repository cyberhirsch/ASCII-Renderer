...
const Game = {
  ...
  // The nearest person. This command was written a phase before there were
  // any, and went on pointing at Entities.list - the creature system, still
  // unbuilt - after the settlements filled with people, so it reported an
  // empty world to somebody standing next to twenty-one of them.
  tpNpc() {
    const live = (typeof NPC === 'undefined') ? [] : NPC.all();
    if (!live.length) {
      // Not a fault in the command: a world whose last people have all
      // ended has nobody left standing in it to walk to.
      this.cmdHistory.push('nobody is left standing in this world - every');
      this.cmdHistory.push('people in the record has ended');
      return;
    }
    let best = null, bd = Infinity;
    for (const n of live) {
    ...
    // Close enough to speak to without being inside them: E reaches 4.5
    // units, and standBy takes the first place there is room to stand.
    this.standBy(best.x, best.y, 2, true);
    this.toast(best.name + ', ' + best.role + '  (' +
               Math.round(Math.sqrt(bd)) + 'u away)  -  E to speak');
  },
